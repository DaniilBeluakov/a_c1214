...
connection = sqlite3.connect("Dz_10_addition_1.sl3", 5)
...
resp = requests.get("https://ua.sinoptik.ua/погода-київ")
...
if resp.status_code == 200:
    soup = BeautifulSoup(resp.text, features="html.parser")
    soup_list = soup.find_all('p')
    soup_list = str(soup_list[24]).split('<p class="Температура">')
    soup_list = str(soup_list[1]).split('</p>')
    temperature = str(soup_list[0])
if resp.status_code == 200:
    soup = BeautifulSoup(resp.text, features="html.parser")
    soup_list = soup.find_all('p')
    soup_list = str(soup_list[2]).split('p class="day-link" data-link="//ua.sinoptik.ua/погода-київ/')
    soup_list = str(soup_list[1]).split('">Неділя</p>')
